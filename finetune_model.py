...
def alexnet(height = 224, width = 224, depth = 3, classes = 1, compile_flag = False):
    AlexNet_model = keras.models.Sequential([
        keras.layers.Conv2D(filters=96, kernel_size=(3,3), strides=(1,1), activation='relu', input_shape=(height, width, depth)),
...
def lenet(height = 224, width = 224, depth = 3, classes = 1, compile_flag = False):
    ...
    LeNet_model.add(Conv2D(input_shape=(height, width, depth), kernel_size=(3, 3), filters=96, strides=(1,1), activation='relu'))
    LeNet_model.add(MaxPooling2D(pool_size=(5, 5), strides=(2, 2)))
    
    # second layer, convolution and pooling
    LeNet_model.add(Conv2D(kernel_size=(3, 3), filters=384, strides=(1,1), activation='relu'))
    LeNet_model.add(MaxPooling2D(pool_size=(5, 5), strides=(2, 2)))

    LeNet_model.add(Conv2D(kernel_size=(3, 3), filters=256, strides=(1,1), activation='relu'))
    
    # Fully connection layer
    LeNet_model.add(Flatten())
    LeNet_model.add(Dense(120, activation = 'relu'))
    LeNet_model.add(Dropout(0.3))
    LeNet_model.add(Dense(840, activation = 'relu'))
    LeNet_model.add(Dropout(0.3))
    LeNet_model.add(Dense(120, activation = 'relu'))
    LeNet_model.add(Dropout(0.3))
    LeNet_model.add(Dense(840, activation = 'relu'))
